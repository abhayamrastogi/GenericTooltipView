//
//  GenericTooltipView.swift
//
//  Created by Abhayam Rastogi on 01/08/21.
//

import UIKit

@IBDesignable
class GenericTooltipView: UIView {
    
    //MARK: - IBInspectable
    
    @IBInspectable var arrowLeftTop: Bool = false
    @IBInspectable var arrowLeftBottom: Bool = false
    @IBInspectable var arrowRightTop: Bool = false
    @IBInspectable var arrowRightBottom: Bool = false
    ...
    // Orientation methods
    private func leftTop(_ x: CGFloat, _ y: CGFloat) -> CGPoint {
        return CGPoint(x: x, y: y)
    }
    
    private func leftBottom(_ x: CGFloat, _ y: CGFloat) -> CGPoint {
        return CGPoint(x: x, y: CGFloat(tooltipHeight) - y)
    }
    
    private func rightTop(_ x: CGFloat, _ y: CGFloat) -> CGPoint {
        return CGPoint(x:  CGFloat(tooltipWidth) - x, y: y)
    }
    
    private func rightBottom(_ x: CGFloat, _ y: CGFloat) -> CGPoint {
    ...
    private func drawTooltip() {
        ...
        // Arrow
        if(arrowLeftTop) {
            bubblePath.move(to: leftTop(0, 0))
            bubblePath.addLine(to: leftTop(-4, 0))
            bubblePath.addQuadCurve(to: CGPoint(x: -4, y: 6), controlPoint: CGPoint(x: -8, y: 3))
            bubblePath.addLine(to: leftTop(0, 13))
            bubblePath.close()
        }
        
        if(arrowLeftBottom) {
            bubblePath.move(to: leftBottom(0, 0))
            bubblePath.addLine(to: leftBottom(-4, 0))
            bubblePath.addQuadCurve(to: CGPoint(x: -4, y: 6), controlPoint: CGPoint(x: -8, y: 3))
            bubblePath.addLine(to: leftBottom(0, 13))
            bubblePath.close()
        }
        
        if(arrowRightTop) {
            bubblePath.move(to: rightTop(0, 0))
            bubblePath.addLine(to: rightTop(-4, 0))
            bubblePath.addQuadCurve(to: CGPoint(x: -4, y: 6), controlPoint: CGPoint(x: -8, y: 3))
            bubblePath.addLine(to: rightTop(0, 13))
            bubblePath.close()
        }
        
        if(arrowRightBottom) {
            bubblePath.move(to: rightBottom(0, 0))
            bubblePath.addLine(to: rightBottom(-4, 0))
            bubblePath.addQuadCurve(to: CGPoint(x: -4, y: 6), controlPoint: CGPoint(x: -8, y: 3))
            bubblePath.addLine(to: rightBottom(0, 13))
            bubblePath.close()
        }
